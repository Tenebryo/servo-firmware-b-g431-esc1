...
#endif
#endif
int32_t TRC_LPF(int32_t in, int32_t *out_buf, int32_t t) //Q15 32767 is max mean 1.000
{
  int32_t x;
...
void PWMC_CalcPhaseCurrentsEst(PWMC_Handle_t *pHandle, qd_t Iqd, int16_t hElAngledpp)
{
  ...
  idq_ave.q = TRC_LPF(Iqd.q, &(pHandle->LPFIqBuf), pHandle->LPFIqd_const);
  idq_ave.d = TRC_LPF(Iqd.d, &(pHandle->LPFIdBuf), pHandle->LPFIqd_const);
  ...
  /*Ib*/
  pHandle->IbEst = (temp1 - temp2)/2;

  /*Ic*/
  pHandle->IcEst = (temp1 + temp2)/2;
